...
export default function BeritaOrganisasiPage() {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .select('link')
      .then(({ data: rows, error }) => {
        if (error) { console.error('Gagal memuat bookmark:', error.message); return }
        setSavedLinks(new Set((rows || []).map(r => r.link)))
      })
  }, [user])
  ...
  const toggleBookmark = async (e: MouseEvent<HTMLButtonElement>, b: BeritaOrganisasi) => {
    e.preventDefault()
    e.stopPropagation()
    if (!user) return
    const sudahTersimpan = savedLinks.has(b.link)
    if (sudahTersimpan) {
      const { error } = await supabase.from('berita_disimpan').delete().eq('link', b.link)
      ...
      const { error } = await supabase.from('berita_disimpan').insert({
        user_id: user.id,
        link: b.link,
        judul: b.judul,
        sumber: b.sumber,
      ...
      if (error) { toast.gagal('Gagal menyimpan bookmark: ' + error.message); return }
      setSavedLinks(prev => new Set(prev).add(b.link))
    }
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <Link
          href="/berita/tersimpan"
          className="shrink-0 px-3 py-1.5 rounded-full text-sm font-semibold whitespace-nowrap bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors flex items-center gap-1.5"
        >
              ...
              <div className="p-5">
                ...
                <div className="flex items-center justify-between mt-3">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      onClick={e => toggleBookmark(e, unggulan)}
                      aria-label={savedLinks.has(unggulan.link) ? 'Hapus dari tersimpan' : 'Simpan berita'}
                      title={savedLinks.has(unggulan.link) ? 'Hapus dari tersimpan' : 'Simpan berita'}
                      className={`pointer-events-auto w-7 h-7 flex items-center justify-center rounded-full transition-colors ${
                        savedLinks.has(unggulan.link)
                          ? 'bg-amber-100 text-amber-600 dark:bg-amber-900/30 dark:text-amber-400'
                          : 'bg-slate-100 text-slate-500 hover:bg-slate-200 dark:bg-slate-700 dark:text-slate-400 dark:hover:bg-slate-600'
                      }`}
                    >
                      <BookmarkIcon className="w-3.5 h-3.5" filled={savedLinks.has(unggulan.link)} />
                    </button>
                    <button
                  ...
                  <div className="relative z-[1] min-w-0 flex-1 flex flex-col pointer-events-none">
                    ...
                    <div className="mt-auto pt-1 flex items-center justify-between gap-2">
                      ...
                      <div className="flex items-center gap-1.5 shrink-0">
                        <button
                          onClick={e => toggleBookmark(e, b)}
                          aria-label={savedLinks.has(b.link) ? 'Hapus dari tersimpan' : 'Simpan berita'}
                          title={savedLinks.has(b.link) ? 'Hapus dari tersimpan' : 'Simpan berita'}
                          className={`pointer-events-auto w-5 h-5 flex items-center justify-center rounded-full transition-colors ${
                            savedLinks.has(b.link)
                              ? 'bg-amber-100 text-amber-600 dark:bg-amber-900/30 dark:text-amber-400'
                              : 'bg-slate-100 text-slate-500 hover:bg-slate-200 dark:bg-slate-700 dark:text-slate-400 dark:hover:bg-slate-600'
                          }`}
                        >
                          <BookmarkIcon className="w-3 h-3" filled={savedLinks.has(b.link)} />
                        </button>
                        <button
